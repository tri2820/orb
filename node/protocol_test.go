...
func TestRegisterMessage(t *testing.T) {
	msg := NewRegisterMsg("msg-001", "node-001")
	...
	if decoded.ControlType() != MsgTypeRegister {
		t.Errorf("Type = %q, want %q", decoded.ControlType(), MsgTypeRegister)
	}
...
func TestAnnounceMessage(t *testing.T) {
	...
	msg := NewAnnounceMsg("msg-002", services)
	...
	if decoded.ControlType() != MsgTypeAnnounce {
		t.Errorf("Type = %q, want %q", decoded.ControlType(), MsgTypeAnnounce)
	}
...
func TestAckMessage(t *testing.T) {
	...
	if decoded.ControlType() != MsgTypeAck {
		t.Errorf("Type = %q, want %q", decoded.ControlType(), MsgTypeAck)
	}
...
func TestOpenBridgeMessage(t *testing.T) {
	service := &Service{ID: "cam1", Addr: "192.168.1.100", Port: 554, Type: "rtsp"}
	msg := NewOpenBridgeMsg("msg-003", "bridge-123", service)
	...
	if decoded.ControlType() != MsgTypeOpenBridge {
		t.Errorf("Type = %q, want %q", decoded.ControlType(), MsgTypeOpenBridge)
	}
...
func TestCloseBridgeMessage(t *testing.T) {
	msg := NewCloseBridgeMsg("msg-004", "bridge-123")
	...
	if decoded.ControlType() != MsgTypeCloseBridge {
		t.Errorf("Type = %q, want %q", decoded.ControlType(), MsgTypeCloseBridge)
	}
	if decoded.Control.BridgeID != "bridge-123" {
		t.Errorf("BridgeID = %q, want %q", decoded.Control.BridgeID, "bridge-123")
	}
...
func TestDataMessage(t *testing.T) {
	...
	msg := NewDataMsg("msg-005", "bridge-123", payload)
...
func TestLargePayload(t *testing.T) {
	...
	msg := NewDataMsg("msg-large", "bridge-123", payload)
...
func TestEmptyServices(t *testing.T) {
	msg := NewAnnounceMsg("msg-empty", []Service{})
...
func TestEmptyPayload(t *testing.T) {
	msg := NewDataMsg("msg-empty-data", "bridge-123", []byte{})
...
func TestMessageHelpers(t *testing.T) {
	...
	controlMsg := NewRegisterMsg("msg-1", "node-1")
	...
	dataMsg := NewDataMsg("msg-2", "bridge-1", []byte("test"))
	...
	// Test ControlType
	if controlMsg.ControlType() != MsgTypeRegister {
		t.Errorf("ControlType = %q, want %q", controlMsg.ControlType(), MsgTypeRegister)
	}
...
func TestLoadAllowList(t *testing.T) {
	allowList, err := LoadAllowList("allowlist.json")
	if err != nil {
	...
	if rtsp.Auth.Type != AuthTypeUsernamePassword {
		t.Errorf("Service[0].Auth.Type = %q, want %q", rtsp.Auth.Type, AuthTypeUsernamePassword)
	}
...
func TestServiceWithPathAndAuth(t *testing.T) {
	...
	service := Service{
		...
		Path: "/live/stream1",
		Auth: &Auth{
			Type:     AuthTypeUsernamePassword,
			Username: "admin",
			Password: "secret",
		},
	}

	msg := NewOpenBridgeMsg("msg-auth", "bridge-auth", &service)
	...
	if svc.Auth.Type != AuthTypeUsernamePassword {
		t.Errorf("Auth.Type = %q, want %q", svc.Auth.Type, AuthTypeUsernamePassword)
	}
...
func BenchmarkEncode(b *testing.B) {
	msg := NewDataMsg("msg-bench", "bridge-123", make([]byte, 1024))
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = msg.Encode()
	}
