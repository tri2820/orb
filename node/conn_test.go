...
func TestConnMultipleMessages(t *testing.T) {
	...
	messages := []*Message{
		NewRegisterMsg("msg-1", "node-1"),
		NewAnnounceMsg("msg-2", []Service{{ID: "svc-1", Addr: "192.168.1.1", Port: 554}}),
		NewDataMsg("msg-3", "bridge-1", []byte("test payload")),
		NewAckMsg("msg-4", "msg-1"),
		NewCloseBridgeMsg("msg-5", "bridge-1"),
	}
...
func TestConnLargeMessage(t *testing.T) {
	...
	msg := NewDataMsg("msg-large", "bridge-1", payload)
	if err := clientConn.WriteMessage(msg); err != nil {
		t.Fatalf("WriteMessage failed: %v", err)
	}
...
func TestConnVeryLargeMessage(t *testing.T) {
	...
	msg := NewDataMsg("msg-verylarge", "bridge-1", payload)
	if err := clientConn.WriteMessage(msg); err != nil {
		t.Fatalf("WriteMessage failed: %v", err)
	}
...
func TestConnBidirectional(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		msg := NewRegisterMsg("msg-1", "node-1")
		if err := clientConn.WriteMessage(msg); err != nil {
		...
		if reply.ControlType() != MsgTypeAck {
	...
	go func() {
		...
		reply := NewAckMsg("ack-1", msg.MsgID)
...
func BenchmarkConnWriteRead(b *testing.B) {
	...
	msg := NewDataMsg("msg-bench", "bridge-1", make([]byte, 1024))
...
func BenchmarkConnLargeMessage(b *testing.B) {
	...
	msg := NewDataMsg("msg-bench", "bridge-1", make([]byte, 64*1024)) // 64KB
